(() => {
    ...
    const SHORTCUTS = {
        ...
        announcePositionAndRemaining: 'B',
        playbackSpeedNormal: 'Z',
        playbackSpeedDown: 'X',
        playbackSpeedUp: 'C'
    };
    ...
    function formatClockTime(seconds) {
        return editor().formatTime(seconds).split('.')[0];
    }

    function formatPlaybackRate(rate) {
        return Number(rate || 1).toFixed(2).replace(/\.00$/, '').replace(/(\.\d)0$/, '$1');
    }

    function setPlaybackRate(rate, { announce = true } = {}) {
        const normalizedRate = Math.max(0.25, Math.min(3, Math.round((Number(rate) || 1) * 10) / 10));
        timeline.playbackRate = normalizedRate;
        el.video.playbackRate = normalizedRate;
        el.video.defaultPlaybackRate = normalizedRate;
        if ('preservesPitch' in el.video) el.video.preservesPitch = true;
        if ('mozPreservesPitch' in el.video) el.video.mozPreservesPitch = true;
        if ('webkitPreservesPitch' in el.video) el.video.webkitPreservesPitch = true;
        if (announce) {
            editor().setStatus('description_subtitle_editor.playback_speed_changed', {
                speed: formatPlaybackRate(normalizedRate)
            });
        }
    }

    function announcePositionAndRemaining() {
        const totalDuration = duration();
        const position = Math.max(0, Math.min(totalDuration, Number(el.video.currentTime) || 0));
        editor().setStatus('description_subtitle_editor.position_and_remaining', {
            position: formatClockTime(position),
            remaining: formatClockTime(Math.max(0, totalDuration - position))
        }, { forceRepeat: true });
    }
    ...
    function handleTimelineKeys(event) {
        ...
        let handled = true;
        // Z/X/C and B are deliberately contextual timeline shortcuts rather than global commands.
        if (shortcutMatches('announcePositionAndRemaining', event)) announcePositionAndRemaining();
        else if (shortcutMatches('playbackSpeedNormal', event)) setPlaybackRate(1);
        else if (shortcutMatches('playbackSpeedDown', event)) setPlaybackRate(timeline.playbackRate - 0.1);
        else if (shortcutMatches('playbackSpeedUp', event)) setPlaybackRate(timeline.playbackRate + 0.1);
        else if (shortcutMatches('announceCurrentTime', event)) announcePosition('description_subtitle_editor.current_position');
